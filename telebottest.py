...
# # 		'User-Agent' : 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.114 Safari/537.36'
# # 	}
# #     question = 'https://www.google.com/search?q=новости '
# # 	response = requests.get(question, headers = HEADERS)
# # 	soup = BeautifulSoup(response.content, 'html.parser')
# # 	text = soup.findAll("div","yuRUbf")

# #     for i in range(len(texts[:4]), -1, -1):
# # 		txt = str(i + 1) + ') ' + texts[i].text
# # 		bot.send_message(message.chat.id, '<a href="{}">{}</a>'.format(text[i]['href'], txt), parse_mode = 'html')
